 ...
 * MimsCanvas.java
 *
 * Created on September 13, 2006, 8:33 AM
 */
...
public class MimsCanvas extends ij.gui.ImageCanvas {
    ...
    /** Creates a new instance of MimsCanvas */
    ...
    void drawOverlay(Graphics g) {
        ...
        if(roiManager.getShowAll() == false ) return ;
        ...
        for(int id = 0 ; id < list.getItemCount() ; id++ ){
            ...
            boolean bDraw = true ;
            if(cRoi != null && cRoi.toString().equals(roi.toString())) bDraw = false;
            if(bDraw) {
              switch(roi.getType()){
                case Roi.COMPOSITE:                    
                    roi.setImage(imp);
