...
public class SegFileUtils {
    ...
    public static void save(MimsRoiManager2 roiManager, String filename, SegmentationProperties props, List<ROIgroup> trainingGroups, int[] groupColors, int colorImageIndex,
            boolean[] massImageFeatures, List<String> ratioImages, int[] localFeatures, byte[] classification, List<ROIgroup> segResultsGroups) {
        try {
            ...
            writeGroups(roiManager, trainingGroups, zos, TRAINPREFIX);
...
//            for (ROIgroup group : setTrainingGroupNames) {
//                trainingGroupsStringArray[j++] = group.getGroupName();
//            }
        
        
            for (int i = 0; i < trainingGroupsStringArray.length; i++) {
                String line = trainingGroupsStringArray[i] + "\t";
                if (groupColors != null && i < groupColors.length) {
                    line += groupColors[i];
                }
                line += "\n";
                bwr.write(line);
...
//            }
            ...
            if (segResultsGroups != null && segResultsGroups.size() > 0) {
 // todo  implement               writeClassManager(segResultsGroups, zos, PREDPREFIX);
            }
    ...
    public static void load(String filename, SegmentationForm form) {
        ...
        ArrayList<Integer> groupColors_tmp = new ArrayList<Integer>();
       ...
       ArrayList<ROIgroup> resultsGroups = new ArrayList<ROIgroup>();
 ...
 // todo  fix                  loadRoi(in, name[1], name[2], trainingGroups);                  
                } else if (name[0].equals(DESCFILE)) {
                    // load description and class colors (if available)
                    String line;
                    while ((line = br.readLine()) != null) {
                        String[] data = line.split("\t");
                        groupNames.add(data[0]);
                        if (data.length > 1) {
                            groupColors_tmp.add(Integer.parseInt(data[1]));
                        }
                    }
    ...
    // todo  fix                 loadRoi(in, name[1], name[2], segResultsGroups);
                }

                entry = in.getNextEntry();
            }

            int[] groupColors = new int[groupColors_tmp.size()];
            for (int i = 0; i < groupColors.length; i++) {
                groupColors[i] = groupColors_tmp.get(i);
            }

            form.setGroupColors(groupColors);
            //form.setClassNames(classNames.toArray(new String[classNames.size()]));
            form.setTrainingGroupNames(groupNames.toArray(new String[groupNames.size()]));
// todo  need to set training a results group name separately?
            ...
            form.setPredictionGroups(resultsGroups);
...
//    }
    ...
    private static void writeGroups(MimsRoiManager2 roiManager, List<ROIgroup> groups, ZipOutputStream zos, String prefix) throws IOException {
        RoiEncoder re = new RoiEncoder(zos);

        // Only do segmentation related groups
        Roi roi;
        int id;
        for (ROIgroup group : groups) {
            String groupName = group.getGroupName();
            String groupType = group.getGroupType();
            if ((groupType.compareTo(MimsRoiManager2.GROUPTYPE_SEGMENTATION_TRAINING) == 0) 
                    || (groupType.compareTo(MimsRoiManager2.GROUPTYPE_SEGMENTATION_RESULT) == 0)) {
                
                // roi = ((Roi) roiManager.getRoiByName(groupName).get(roiManager.getRoiList().getModel().getElementAt(i)));
                 roi = ((Roi) roiManager.getRoiByName(groupName));
                 id = roi.getImageID();
                String label = prefix + SEPARATOR + groupName + SEPARATOR + id;
...
//        }
    ...
    private static void writeFeatures(ZipOutputStream zos, int colorImageIndex, boolean[] massImageFeatures, List<String> ratioImages, int[] localFeatures) throws IOException {
